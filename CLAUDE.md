...
3. **Verify — run the ASVL loop** (defined under Working protocols): fix any
   *flaws* (doesn't do what was asked) and *regressions* (broke something that
   worked), repeating run→observe→fix until it's clean.
...
## When you add a publishing feature — keep the toolkit (skill + command) in sync
...
published by plain request *or* command. Sync **two layers**:

**Layer 1 — the skill (the path):**
...
- **Only if genuinely different — create a new skill** under `.claude/skills/`. If
  its publishing path is truly different (a different mechanism/flow, not just new
  fields), give it its own skill.

**Layer 2 — the command (the button):**

- If you **extended `daleel-publish`**, the existing **`/publish`** command already
  reaches the new variation — nothing to add.
- If you **created a new skill**, add a **matching `/command`** in `.claude/commands/`
  so the new path has a one-tap button too.

**Why:** the app, the skills, *and* the commands must grow together. A new publish
path that ships without **skill** coverage leaves a future Claude unable to drive it;
without **command** coverage it loses its quick button. Keep all three in step.
...
- **Reuse the shared helpers** you saw on the tour (`save_upload`/`take_upload`,
  `_ordered`/`get_one`, `admin_required`) — don't reinvent what `app.py` provides.
