...
(function () {
  ...
  function initSection(section) {
    ...
    if (grid) grid.addEventListener('pointerdown', function (e) {
      ...
      // On TOUCH, only start from the ⠿ handle so the page can still be scrolled by
      // touching the card body. Mouse/pen can still grab anywhere on the card.
      ...
      card.style.transition = 'none';   // instant follow during drag (overridden on drop)
    ...
    function loop() {
      ...
      curY += (tgtY - curY) * 0.3;
      var vx = tgtX - curX;
      // large full-width (vertical) cards: keep it SIMPLE — no tilt, gentler lift,
      // and no dark motion-trail (which turns into a big blackish smear on big cards).
      var tTilt = vertical ? 0 : Math.max(-7, Math.min(7, vx * 0.11));
      tilt += (tTilt - tilt) * 0.18;
      if (Math.abs(tilt) < 0.04) tilt = 0;
      var sc = vertical ? 1.02 : 1.045;
      drag.style.transform =
        'translate3d(' + curX.toFixed(2) + 'px,' + curY.toFixed(2) + 'px,0) rotate(' +
        tilt.toFixed(2) + 'deg) scale(' + sc + ')';

      var now = performance.now();
      if (!vertical && Math.abs(vx) > 7 && now - lastTrail > 60) {
        lastTrail = now;
    ...
    function onUp() {
      ...
      d.style.transform = 'translate3d(' + pr.left + 'px,' + pr.top + 'px,0) rotate(0deg) scale(1)';
      setTimeout(function () {
